@extends($chemin_theme_actif,['title' => 'Depenses-Par-Arrivage'])
    @section('content')
        <div class="page-bar">
            <div class="page-title-breadcrumb">
                <div class=" pull-left">
                    <div class="page-title">Liste Des Dépenses Par Arrivage</div>
                </div>
                <ol class="breadcrumb page-breadcrumb pull-right">
                    <li><i class="fa fa-home"></i>&nbsp;<a class="parent-item"
                            href="{{ route('home') }}">Accueil</a>&nbsp;<i class="fa fa-angle-right"></i>
                    </li>
                    <li><a class="parent-item" href="#">Dépense</a>&nbsp;<i class="fa fa-angle-right"></i>
                    </li>
                    <li class="active">Liste Des Dépenses Par Arrivage</li>
                </ol>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="card card-topline-red">
                    <div class="card-head">
                        <header>Liste Des Dépenses Par Arrivage</header>
                        <div class="tools">
                            ...
                            <div class="col-md-6 col-sm-6 col-6">
                                ...
                                <div class="btn-group">
                                    <button id="addRow1" data-toggle="modal" data-target="#all_niveau" class="btn btn-primary">
                                        <i class="fa fa-list"></i> Afficher Les Depenses Par Arrivage
                                    </button>
                                </div>
                                <!-- debut modal -->
                                ...
                                <tbody>
                                    @foreach ($all_depenses as $depense)
                                        <tr class="odd gradeX">
                                            ...
                                            <td class="text-center">{{ $depense->nom_depense }}</td>
                                            <td class="text-center">{{ $depense->montant.' GNF' }}</td>
                                            <td class="text-center">{{ $depense->date_depense->format('d/m/Y') }}</td>
                                            <td>
                                                <button class="btn btn-primary" data-toggle="modal" data-target="#classes{{ $depense->id }}">Modifier
                                                    <i class="fa fa-edit"></i>
                                                </button>
                                                <!-- debut modal -->
                                                    <div class="modal fade" data-backdrop="static" id="classes{{ $depense->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
                                                        aria-hidden="true">
                                                        <div class="modal-dialog" role="document">
                                                            <div class="modal-content">
                                                                <div  class="modal-header btn btn-danger text-center text-white">
                                                                ...
                                                                <!-- start modal body -->
                                                                    <div class="modal-body">
                                                                        <form action="{{ route('depense.update',$depense->id) }}" method="post" enctype="multipart/form-data">
                                                                            ...
                                                                            <div class="form-group">
                                                                                <label for="nom_depense">Dépense</label>
                                                                                <input type="text" name="nom_depense" id="nom_depense" class="form-control" value="{{ $depense->nom_depense }}" placeholder="Entrez le nom de la dépense" required>
                                                                            </div>
                                                                            <div class="form-group">
                                                                                <label for="montant">Montant</label>
                                                                                <input type="text" name="montant" id="montant" class="form-control" value="{{ $depense->montant }}" placeholder="Entrez le montant" required>
                                                                            </div>
                                                                            <div class="form-group">
                                                                                <label for="date_depense">Date</label>
                                                                            ...
                                                                            <div class="form-group">
                                                                                <label for="voiture_id">Arrivage</label>
                                                                                    <select class="form-control" name="arrivage_id" id="arrivage_id">
                                                                                        <option value="">Sélectionnez un arrivage</option>
                                                                                        @foreach ($all_arrivages as $arrivage)
                                                                                            <option value="{{ $arrivage->id }}" @if($arrivage->id == $depense->arrivage_id) selected @endif>{{ $arrivage->nom_arrivage }}</option>
                                                                                        @endforeach
                                                                                    </select>
                                                                            </div>
                                                                            <div class="modal-footer d-flex justify-content-center">
                        ...
                        <div class="col-md-12">
                                    ...
                                    <div class="cercle">
                                        <div class="text-center">
                                            <h4 class="font-bold addr-font-h4">
                                                <u class="souligner">Liste De Toute Les</u><br>
                                                <u class="souligner">De L'arrivage ...</u>
                                            </h4>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div>
                                <table id="bordure_table" class="table table-bordered">
                                    ...
                                    <th class="text-center" id="bordure_table"> Dépense </th>
                                    ...
                                    <tbody id="bordure_table">
                                        @foreach ($all_depenses as $depense)
                                            <tr id="bordure_table">
                                                ...
                                                <td class="text-center" id="bordure_table">{{ $depense->nom_depense }}</td>
                                                <td class="text-center" id="bordure_table">{{ $depense->montant }}</td>
                                                <td class="text-center" id="bordure_table">{{ $depense->date_depense->format('d/m/Y') }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
