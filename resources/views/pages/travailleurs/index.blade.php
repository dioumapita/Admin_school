...
@section('content')
    <div class="page-bar">
        <div class="page-title-breadcrumb">
            <div class=" pull-left">
                <div class="page-title">Liste Des Travailleurs</div>
            </div>
            <ol class="breadcrumb page-breadcrumb pull-right">
                ...
                <li class="active">Liste Des Travailleurs</li>
            </ol>
        </div>
    </div>
    <div class="row">
                                                ...
                                                <tbody>
                                                    @foreach ($all_travailleurs as $travailleur)
                                                        ...
                                                        <tr class="odd gradeX">
                                                            ...
                                                            <td class="text-center">{{ $travailleur->nom }}</td>
                                                            <td class="text-center">{{ $travailleur->prenom }}</td>
                                                            <th class="text-center">{{ $travailleur->contact }}</th>
                                                            <th class="text-center">{{ $travailleur->quartier }}</th>
                                                            <td>
                                                                <button class="btn btn-primary" data-toggle="modal" data-target="#classes{{ $travailleur->id }}">Modifier
                                                                    ...
                                                                    <div class="modal fade" data-backdrop="static" id="classes{{ $travailleur->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
                                                                        ...
                                                                        <div class="modal-dialog" role="document">
                                                                            <div class="modal-content">
                                                                                ...
                                                                                <!-- start modal body -->
                                                                                    <div class="modal-body">
                                                                                        <form action="{{ route('travailleur.update',$travailleur->id) }}" method="post" enctype="multipart/form-data">
                                                                                            {{ csrf_field() }}
                                                                                            {{ method_field('PUT') }}
                                                                                            <div class="form-group">
                                                                                                <label for="nom">Nom</label>
                                                                                                <input type="text" name="nom" id="nom" class="form-control" value="{{ $travailleur->nom }}" placeholder="Nom" required>
                                                                                            </div>
                                                                                            <div class="form-group">
                                                                                                <label for="prenom">Prénom</label>
                                                                                                <input type="text" name="prenom" id="prenom" class="form-control" value="{{ $travailleur->prenom }}" placeholder="Prénom" required>
                                                                                            </div>
                                                                                            <div class="form-group">
                                                                                                <label for="contact">Contact</label>
                                                                                                <input type="text" name="contact" id="contact" class="form-control" value="{{ $travailleur->contact }}" placeholder="Contact" required>
                                                                                            </div>
                                                                                            <div class="form-group">
                                                                                                <label for="quartier">Quartier</label>
                                                                                                <input type="text" name="quartier" id="quartier" class="form-control" value="{{ $travailleur->quartier }}" placeholder="quartier" required>
                                                                                            </div>
                                                                                            <div class="form-group">
                                                                                                <label for="image_travailleur">Modifier la photo</label>
                                                                                                <input type="file" name="image_travailleur" id="image_travailleur" class="form-control-file" value="{{ old('image_travailleur')}}" placeholder="Modifier la photo">
                                                                                            </div>
                                                                                            <div class="modal-footer d-flex justify-content-center">
                                                                                                <button type="submit" class="btn btn-primary" >Valider <i class="fa fa-check"></i></button>
                                                                                                &nbsp;&nbsp;
                                                                ...
                                                                <!-- fin modal -->
                                                                <a href="#myModal" data-toggle="modal" onclick="deleteData({{ $travailleur->id }})"
                                                                    ...
                                                                    <div class="mt-5 modal-dialog modal-confirm">
                                                                        <div class="modal-content">
                                                                            ...
                                                                            <div class="modal-footer justify-content-center">
                                                                                ...
                                                                                <form action="{{ route('travailleur.destroy',$travailleur->id) }}" method="post" id="deleteform">
                                                                                    {{ csrf_field() }}
                                                                                    {{ method_field('DELETE') }}
                                    ...
                                    <div class="card card-box">
                                        <div class="card-body no-padding ">
                                            <div class="doctor-profile">
                                                <img src="/images/photos/travailleurs/{{ $travailleur->image_travailleur }}" class="doctor-pic" width="150px" height="150px"
                                                    alt="ndsplus-travailleur">
                                                <div class="profile-usertitle">
                                                    <div class="doctor-name">{{ $travailleur->nom.' '.$travailleur->prenom }} </div>
                                                </div>
                                                <div>
                                                    <p>
                                                        <i class="fa fa-phone"></i> Contact: {{ $travailleur->contact }}<br>
                                                        <i class="fa fa-home"></i> Ville: {{ $travailleur->quartier }}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
...
<script>
    function deleteData(id)
         {
             var id = id;
             var url = '{{ route("travailleur.destroy", ":id") }}';
